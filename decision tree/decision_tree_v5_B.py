...
def select_best_feature_to_split(dataset, attributes):
    ...
    for i in range(len(attributes)):
        ...
        # Deal with continuous features # should be 2, 3, 6, 7
        if type(feature_list[0]) == 'float' or type(feature_list[0]) == 'int':
...
def load_data(filename):
    ...
    with codecs.open(filename, "r") as csvfile:
        ...
        for row in csvReader:
            # print(row)
            if filename == "csv_result-trainProdIntro-binary.csv" or\
                            filename == "csv_result-testProdIntro-test-binary.csv":
                label_list.append(row[0].split(',')[9])
                dataset.append(row[0].split(',')[1:9])
            elif filename == "csv_result-trainProdSelection_A.csv":
                label_list.append(row[0].split(',')[7])
                dataset.append(row[0].split(',')[1:7])

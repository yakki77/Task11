...
# max = [15.00, 7.15, 15]
# interval = []
# number_of_cts_featrues = len(min)
# print(number_of_cts_featrues)
#
# for i in range(0, number_of_cts_featrues - 1):
#     print(i)
#     interval[i] = (int(max[i]) - int(min[i])) / float(5)
# print(interval)
#
# min_val=0.00; max_val=15.00; num_bin=5
# interval=(max_val-min_val)/float(5)

# def demo_reg(dataset):
#     pass
#     count = 0
#     for now_ins in dataset:
...
#             tmp = float(now_ins[i])
#             new_val = (tmp-min[i])
#             new_val = int(new_val/interval[i])
#             now_ins[i] = str(new_val)
#             dataset[count] = now_ins[i]
#             count += 1
#
#     return dataset
...
def main():
    ...
    for i in range(len(prediction)):
        if prediction[i] == label_list_test[i]:
            count += 1
    print("Accuracy for test data is " + str(count/len(prediction)))
